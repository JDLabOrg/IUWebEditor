...
- (void) setDefaults {
    ...
    openedFiles = [[NSMutableArray alloc] init];
    tabOffset = -15;
    startingOffset = 10;
    tabMagneticForce = 5;
...
- (void)mouseDown: (NSEvent *) theEvent {
    ...
    if([clickedLayer isKindOfClass:[SFCloseLayer class]]){
        //close file tab
        if(currentSelectedTab == [clickedLayer superlayer]){
            [self selectPreviousTab];
        }
        [self removeTab:[clickedLayer superlayer]];
    }
    else if (clickedLayer &&  clickedLayer != tabsLayer ) {
        canDragTab = NO;
...
- (void) addTabWithRepresentedObject: (id) representedObject {
     MGFileItem *item = (MGFileItem *)representedObject;
   
    if([openedFiles containsObject:item]){
        NSUInteger index =  [openedFiles indexOfObject:item];
        [self selectTabAtIndex:index];
            
    }
    else{
        [openedFiles addObject:item];
         NSDictionary * currentItem = [NSDictionary dictionaryWithObjects:@[item, item.name] forKeys:@[@"fileItem", @"name"]];
        [self addTabAtIndex:[self numberOfTabs] withRepresentedObject:currentItem];
        [self selectLastTab];
    }
}

- (void) addTabAtIndex: (NSUInteger)index withRepresentedObject: (id) representedObject {
...
-(void)removeTabOfItem:(MGFileItem *)item{
    NSUInteger index =  [openedFiles indexOfObject:item];
    if(index >= [openedFiles count]) return;
...
- (void) removeTab: (CALayer *) tab {
    ...
    NSUInteger tabIndex = [self indexOfTab:tab];
    if (tabIndex != -1) {
...
- (void) removeTabAtIndex: (NSUInteger) index {
    ...
    // Grabbing the tab.
    NSUInteger indexOfInitialTab = index;
    CALayer *tab = [arrangedTabs objectAtIndex:indexOfInitialTab];
    CGPoint startingOrigin = tab.frame.origin;
    NSUInteger indexOfLandingTab = [arrangedTabs count] -1;
    
	
    NSUInteger newIndex = indexOfInitialTab; //- 1;
    ...
    [openedFiles removeObjectAtIndex:index];
...
- (NSUInteger) indexOfTab: (CALayer *) tab {
...
- (CALayer *) tabAtIndex: (NSUInteger) index {
...
- (void) selectNextTab{
    NSUInteger currentTabIndex = [ self indexOfTab: [self selectedTab] ];
    NSUInteger nextIndex = currentTabIndex + 1;
...
- (void) selectPreviousTab{
    NSUInteger currentTabIndex = [ self indexOfTab: [self selectedTab] ];
    NSUInteger prevIndex = currentTabIndex - 1;
...
- (void) rearrangeInitialTab: (CALayer *) initialTab toLandingTab:(CALayer *) landingTab withCurrentPoint: (CGPoint) currentPoint direction: (BOOL) direction{
    NSUInteger indexOfInitialTab = [self indexOfTab:initialTab];
    NSUInteger indexOfLandingTab = [self indexOfTab:landingTab];
    ...
    for(NSNumber *n in tabsSequence){
        ...
        [openedFiles removeObjectAtIndex:indexOfInitialTab];
        [openedFiles insertObject:[[((SFDefaultTab *)initialTab) representedObject] valueForKey:@"fileItem" ] atIndex:newIndex];
...
- (NSArray *) tabSequenceForStartingTabIndex: (NSUInteger) startingIndex endingTabIndex: (NSUInteger) endingIndex direction: (BOOL) direction {
    NSMutableArray *tagsSequence = [NSMutableArray array];
    
    for (NSUInteger i = MIN(startingIndex, endingIndex); i<=MAX(startingIndex, endingIndex); i++) {
        if (i == startingIndex)
            continue;
        else if (direction)
            [tagsSequence addObject:[NSNumber numberWithUnsignedInteger:i]];
        else
            [tagsSequence insertObject:[NSNumber numberWithUnsignedInteger:i] atIndex:0];
    }
...
- (int) startingXOriginForTabAtIndex: (NSUInteger) index {
    if (index == 0)
